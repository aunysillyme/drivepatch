...
var SQUARES=[
 {t:"A family of five &middot; warm coats, two children under six",s:"wait",days:1},
 {t:"A grandmother raising two &middot; groceries for the week",s:"held",by:"Maria",note:"dropping off Thursday",f:1,k:2},
 {t:"A teenager &middot; nobody has claimed anything for this age yet",s:"wait",days:2},
 ...
 {t:"Two sisters, 9 and 11 &middot; a warm bed each",s:"wait",days:1},
 {t:"An older couple &middot; heating oil and a hot meal",s:"held",by:"St Anne's",note:"delivered",f:4,k:3},
 {t:"A family of three &middot; nothing specified, they didn't want to ask",s:"wait",days:2},
 {t:"A mum on her own &middot; a winter coat, size 12",s:"held",by:"the Reyes family",note:"packed",f:5,k:1},
 {t:"A boy of seven who loses track of time building things",s:"wait",days:0},
 {t:"Someone living alone &middot; a hot meal and company",s:"held",by:"Tomas",note:"delivered",f:6,k:6},
 {t:"A family of six &middot; bedding, and the heating is off",s:"wait",days:1}
];
...
function pace(){
  ...
  var elapsed=3;                        /* days the drive has been open */
...
function stuck(){return waiting().filter(function(d){return (d.days||0)>=2;});}
function starved(){
  var by={};
...
function waitLadder(){
  var buckets=[0,0,0,0];   /* same day, 1 day, 2 days, 3+ days */
  waiting().forEach(function(d){var v=d.days||0;
    buckets[v<1?0:v<2?1:v<3?2:3]++;});
  var max=Math.max.apply(null,buckets)||1;
  var lab=["same day","1 day","2 days","3 days or more"];
  ...
  return '<div class="sect">the progress board</div>'+
    '<div class="pgrid">'+
      ...
      '<div class="pcard"><div class="pk">nobody has touched these</div>'+
        ...
        '<p>'+(st.length?'Waiting two days or more with no volunteer. '+
  ...
  return '<h1 style="font-size:34px;margin-bottom:6px">The quilt today, square by square.</h1>'+
    ...
    longest.map(function(d){
      return '<div class="row"><div class="when">'+(d.days||0)+' days</div><div class="body">'+
        ...
        '<span class="pill '+((d.days||0)>=2?"late":"wait")+'">'+((d.days||0)>=2?"chase this":"waiting")+'</span></div></div>';}).join("")+
...
/* ---------------- faded hearts behind everything ----------------
   ...
   renders. The whole layer sits at 15% opacity (svg.hearts in style.css), so
   each heart is drawn solid and the layer does the fading. Auny's call. */
function heartField(){
  var seed=20261128;
